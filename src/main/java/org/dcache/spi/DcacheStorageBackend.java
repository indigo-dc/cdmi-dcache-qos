...
public class DcacheStorageBackend implements StorageBackend {
  ...
  static {
    ...
    exports.put(
        "Network/WebHTTP",
        new JSONObject()
            .put("identifier", "https://dcache-qos-01.desy.de/")
            .put("permissions", "oidc"));
  }

  private PluginConfig config;
  private String dcacheServer;
  ...
  public DcacheStorageBackend() {
    ...
    isRestApiNew = config.get("cdmi.dcache.rest.version").equals("new");
    scheme = config.get("dcache.server.rest.scheme") + "://";

    dcacheServer =
        scheme
            + config.get("dcache.server")
            + ":"
            + config.get("dcache.server.rest.endpoint")
            + "/";
  ...
  public List<BackendCapability> getCapabilities() throws BackEndException {
    String url = dcacheServer + apiPrefix + qosPrefixOld;
    try {
  ...
  @VisibleForTesting
  JSONObject updateCdmiObjectStub(String path, String targetCapabilityUri) throws BackEndException {
    String url =
        dcacheServer
            + apiPrefix
            + (isRestApiNew ? qosPrefixNew : "qos-management/" + "namespace")
            + path;
    try {
      ...
      post.setEntity(
          new StringEntity(
              isRestApiNew
                  ? JsonUtils.targetCapUriToJsonNew(targetCapabilityUri)
      ...
      LOG.error(
          "Error Updating Capability of {} to {}: {}", path, targetCapabilityUri, se.getMessage());
      throw new BackEndException(se.getMessage(), se.getCause());
    } catch (UnsupportedEncodingException | IllegalArgumentException ue) {
      LOG.error(
          "Error creating request to update capability of {} to {}: {}",
          path,
          targetCapabilityUri,
          ue.getMessage());
  ...
  public CdmiObjectStatus getCurrentStatus(String path) throws BackEndException {
    ...
    String childUrl = dcacheServer + apiPrefix + "namespace" + path + "/?children=true";
    ...
    try {
      ...
      String currentCapUrl =
          HttpUtils.getCapabilityUri(
              dcacheServer + apiPrefix + qosPrefixOld, rest.getString("fileType"), curQos);

      final Map<String, Object> monitored = HttpUtils.monitoredAttributes(currentCapUrl);
      LOG.debug("Children {}  of Cdmi object {}", childrenJson, path);

      final List<String> children = ParseUtils.extractChildren(childrenJson);

      String currentCapUri =
          "/cdmi_capabilities/"
      ...
      LOG.info(
          "Cdmi Capability of object {} is {}; in transition to {}",
          path,
          currentCapUri,
          targetCapUri);
